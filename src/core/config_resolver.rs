...
use crate::models::{
    GlobalIndex, IndexEntry, ProjectConfig, ResolvedConfig,
    SerializableConfigCache, OptionsConfig,
};
...
pub fn resolve_config_for_uuid(
    ...
    let leaf_entry = index.projects.get(&target_uuid)
    ...
    if let Some(cached_config) = read_and_validate_config_cache(&config_cache_path, &qualified_name)? {
        log::debug!("Caché de configuración válido encontrado para '{}'.", qualified_name);
        return Ok(cached_config);
    }
    log::debug!("Caché de config inválido o no encontrado. Resolviendo '{}'...", qualified_name);
    ...
    log::debug!("Nuevo caché de config guardado en '{}'.", config_cache_path.display());
...
fn build_inheritance_chain<'a>(
    ...
    while let Some(current_uuid) = current_uuid_opt {
        let entry = index.projects.get(&current_uuid)
...
fn load_project_config(entry: &IndexEntry) -> ResolverResult<ProjectConfig> {
    ...
    if !config_path.is_file() {
        return Err(ResolverError::ConfigFileNotFound {
            name: entry.name.clone(), path: config_path.display().to_string()
        });
    }
    let content = fs::read_to_string(&config_path)?;
    toml::from_str(&content).map_err(|e| ResolverError::TomlParse {
        path: config_path.display().to_string(), source: e
    })
}

fn read_and_validate_config_cache(cache_path: &Path, expected_name: &str) -> ResolverResult<Option<ResolvedConfig>> {
    ...
    for (path_str, cached_mod_time_serializable) in serializable_cache.dependencies.iter() {
        ...
        if !path.exists() {
            log::debug!("Dependencia de caché '{}' no existe. Caché inválido.", path.display());
            return Ok(None);
        }
        ...
        if current_mod_time > cached_mod_time {
            log::debug!("Dependencia de caché '{}' ha sido modificada. Caché inválido.", path.display());
            return Ok(None);
        }
    }
